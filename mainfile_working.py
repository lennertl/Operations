'''
Simplifications: only 1 veh, only daytime is considered atm, same time windows, same customer demands, same service time, same time in the day
'''
...
instance_name = 'grid11.csv'

distance_name = 'distances11.csv'
...
token = 1		# [kg], dummy mass to force return to depot
...
# Number of vehicles
nr_veh = int(math.ceil(((len(lh)-1)*a + (len(bh))*b)/q)) # depot incl in number of backhaul nodes since depot is bh
...
for i in lh:
	for k in range(nr_veh):
		u[i,k] = model.addVar(lb=0, ub=1, vtype=GRB.BINARY, name="u[%s,%s]"%(i,k))

# Backhaul customers
v = {}
for i in bh:
	for k in range(nr_veh):
		v[i,k] = model.addVar(lb=0, ub=1, vtype=GRB.BINARY, name="v[%s,%s]"%(i,k))	
...
for i in range(len(customers)):
	for j in range(len(customers)):
		for k in range(nr_veh):
			f[i,j,k] = model.addVar(lb=0, ub=q, vtype=GRB.INTEGER, name="f[%s,%s,%s]"%(i,j,k))
...
for i in lh[1:]:
	...
	for k in range(nr_veh):
		thisLHS += u[i,k]
	model.addConstr(lhs=thisLHS, sense=GRB.EQUAL, rhs=1, name='customer_served_lh_' + str(i) + '_veh_' + str(k))
...
for i in bh[1:]:
	...
	for k in range(nr_veh):
		thisLHS += v[i,k]
	model.addConstr(lhs=thisLHS, sense=GRB.EQUAL, rhs=1, name='customer_served_bh_' + str(i) + '_veh_' + str(k))
...
for k in range(nr_veh):
	...
	for i in lh[1:]:
		thisLHS += a*u[i,k]
	model.addConstr(lhs=thisLHS, sense=GRB.LESS_EQUAL, rhs=q, name='vehicle_cap_lh_veh_' + str(k))
...
for k in range(nr_veh):
	...
	for i in bh[1:]:
		thisLHS += b*v[i,k] 
	model.addConstr(lhs=thisLHS, sense=GRB.LESS_EQUAL, rhs=q, name='vehicle_cap_bh_veh_' + str(k))
...
for k in range(nr_veh):
	for j in range(len(customers)):
		if j in lh[1:]:
			...
			for i in range(len(customers)):
				thisLHS += x[i,j,k] 
			model.addConstr(lhs=thisLHS, sense=GRB.EQUAL, rhs=u[j,k], name='flow_arrival_lh_customer_' + str(j) + '_veh_' + str(k))
		if j in bh:
			thisLHS = LinExpr()
			for i in range(len(customers)):
				thisLHS += x[i,j,k]
			model.addConstr(lhs=thisLHS, sense=GRB.EQUAL, rhs=v[j,k], name='flow_arrival_bh_customer_' + str(j) + '_veh_' + str(k))
...
for k in range(nr_veh):
	for i in range(len(customers)):
		if i in lh:
			...
			for j in range(len(customers)):
				thisLHS += x[i,j,k]
			model.addConstr(lhs=thisLHS, sense=GRB.EQUAL, rhs=u[i,k], name='flow_departure_lh_customer_' + str(i) + '_veh_' + str(k))
		if i in bh[1:]:
			thisLHS = LinExpr()
			for j in range(len(customers)):
				thisLHS += x[i,j,k]
			model.addConstr(lhs=thisLHS, sense=GRB.EQUAL, rhs=v[i,k], name='flow_departure_bh_customer_' + str(i) + '_veh_' + str(k))
...
for k in range(nr_veh):
	...
	for i in lh:
		for j in bh:
			thisLHS += x[i,j,k]
	model.addConstr(lhs=thisLHS, sense=GRB.EQUAL, rhs=1, name='lh_priority_veh_' + str(k))
...
for k in range(nr_veh):
	for j in range(1,len(customers)):		# start time for arrival node
		for i in range(1,len(customers)):	# start time for departure node
			thisLHS = LinExpr()
			thisLHS += t[j] - t[i] - s - daytime[i][j] + (1-x[i,j,k])*R
			model.addConstr(lhs=thisLHS, sense=GRB.GREATER_EQUAL, rhs=0, name='start_time_dep_' + str(i) + '_arr_' + str(j) + '_veh_' + str(k))
...
for k in range(nr_veh):
	for i in range(1,len(customers)):
		if i in lh[1:]:
			...
			for j in range(len(customers)):
				if i != j:
					thisLHS += f[j,i,k]*x[j,i,k] - f[i,j,k]*x[i,j,k]		# sum of sum elements = sum elements of sum (linear)
			model.addConstr(lhs=thisLHS, sense=GRB.EQUAL, rhs=a, name='load_flow_balance_lh_' + str(i) + '_veh_' + str(k))
		if i in bh[1:]:
			thisLHS = LinExpr()
			for j in range(len(customers)):
				if i != j:
					thisLHS += f[j,i,k]*x[j,i,k] - f[i,j,k]*x[i,j,k]
			model.addConstr(lhs=thisLHS, sense=GRB.EQUAL, rhs=b, name='load_flow_balance_bh_' + str(i) + '_veh_' + str(k))

for k in range(nr_veh):
	thisLHS = LinExpr()
	for j in range(1,len(customers)):
		thisLHS += f[0,j,k]*x[0,j,k]
	model.addConstr(lhs=thisLHS, sense=GRB.EQUAL, rhs=q, name='forced_depot_output_veh_' + str(k))

for k in range(nr_veh):
	thisLHS = LinExpr()
	for i in range(1,len(customers)):
		thisLHS += f[i,0,k]*x[i,0,k]
	model.addConstr(lhs=thisLHS, sense=GRB.GREATER_EQUAL, rhs=0, name='forced_depot_input_veh_' + str(k))
'''
# Equation 15
for k in range(nr_veh):
	for j in range(1,len(customers)):
		for i in range(1,len(customers)):
			model.addConstr(lhs=a*x[i,j,k], sense=GRB.LESS_EQUAL, rhs=f[i,j,k], name='load_cap_lh_lhs_rte_' + str(i) + '_' + str(j) + '_veh_' + str(k))
			model.addConstr(lhs=f[i,j,k], sense=GRB.LESS_EQUAL, rhs=(q-a)*x[i,j,k], name='load_cap_lh_rhs_rte_' + str(i) + '_' + str(j) + '_veh_' + str(k))

# Equation 16
for k in range(nr_veh):
	for j in range(1,len(customers)):
		for i in range(1,len(customers)):
			model.addConstr(lhs=b*x[i,j,k], sense=GRB.LESS_EQUAL, rhs=f[i,j,k], name='load_cap_bh_lhs_rte_' + str(i) + '_' + str(j) + '_veh_' + str(k))
			model.addConstr(lhs=f[i,j,k], sense=GRB.LESS_EQUAL, rhs=(q-b)*x[i,j,k], name='load_cap_bh_rhs_rte_' + str(i) + '_' + str(j) + '_veh_' + str(k))
...
for k in range(nr_veh):	
	model.addConstr(lhs=u[0,k], sense=GRB.EQUAL, rhs=1, name='depot_forced_lh_node_veh_' + str(k))
	model.addConstr(lhs=v[0,k], sense=GRB.EQUAL, rhs=1, name='depot_forced_bh_node_veh_' + str(k))
...
for i in range(len(customers)):
	for j in range(len(customers)):
		for k in range(nr_veh):
			obj += (alpha*distance[i][j]*(w + f[i,j,k]) + beta*V**2*distance[i][j])*x[i,j,k]
...
# color gen for plots
color = iter(cm.rainbow(np.linspace(0, 1, k+1)))
...
for k in range(nr_veh):
	...
	for i in range(len(customers)):
		for j in range(len(customers)):
			if x[i,j,k].x:
				plt.plot([x_coord[i], x_coord[j]], [y_coord[i], y_coord[j]], c=c)
